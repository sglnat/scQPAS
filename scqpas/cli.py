...
def main(config, bam, gtf, chr, pas, output, percentage_threshold, length_threshold, use_fc, log_level, log_file):
    """
    ...
    try:
        ...
        if percentage_threshold is None:
            percentage_threshold = config_manager.get('polya_detection', 'percentage_threshold', 80)
        
        if length_threshold is None:
            length_threshold = config_manager.get('polya_detection', 'length_threshold', 5)
        
        # use_fc is a flag, so it's False by default in Click
        if not use_fc:
            use_fc = config_manager.get('polya_detection', 'use_fixed_coordinates', False)

...
def _extract_n_operations_from_cigar(cigar_str: str) -> tuple:
    """
    Extract N operation lengths from a CIGAR string.

    Helper function that parses a single CIGAR string and extracts all N operations.
    Used by both get_cigar_bed() (for interval positions) and extract_cigar_n_metrics()
    (for count/sum aggregation).

    Parameters
    ----------
    cigar_str : str
        CIGAR string (e.g., "100M50N200M")

    Returns
    -------
    tuple of two elements:
        - list of int: lengths of each N operation
        - list of (operation, length) tuples for full CIGAR parsing
    """
    if pd.isna(cigar_str):
        return [], []

    cig_segments = re.findall(r"(\d+)([MIDNSHPX=])", str(cigar_str))
    n_operations = []

    for length_str, operation in cig_segments:
        length = int(length_str)
        if operation == "N":
            n_operations.append(length)

    return n_operations, cig_segments


def get_cigar_bed(
    df_input: Union[pd.DataFrame, str], bed_output: Optional[str] = None
) -> pd.DataFrame:
    """
    ...
    # Vectorized CIGAR parsing using apply
    def parse_cigar_row(row):
        """Extract N introns from a single CIGAR string"""
        introns = []
        _, cig_segments = _extract_n_operations_from_cigar(row["CIGAR"])
...
def extract_cigar_n_metrics(reads_df: pd.DataFrame) -> pd.DataFrame:
    """
    Extract CIGAR metrics (N operation count and total length) for each read.

    Uses the same CIGAR parsing logic as get_cigar_bed() via the shared helper
    _extract_n_operations_from_cigar(). Extracts:
    - CIGAR_N: Count of N operations in the CIGAR string
    - total_cigar_N_length: Sum of all N operation lengths

    This is used for polyA reads where we need to subtract gap lengths from distances,
    and for tracking intronic read information in general.

    Parameters
    ----------
    reads_df : pd.DataFrame
        DataFrame with 'read_id' and 'CIGAR' columns

    Returns
    -------
    pd.DataFrame
        DataFrame with columns: ['read_id', 'CIGAR_N', 'total_cigar_N_length']
        All reads present in input are present in output (reads with no N operations
        have CIGAR_N=0 and total_cigar_N_length=0).
    """

    def _metrics_from_cigar(cigar_str):
        """Extract N operation count and total length from CIGAR string"""
        n_operations, _ = _extract_n_operations_from_cigar(cigar_str)
        return (len(n_operations), sum(n_operations)) if n_operations else (0, 0)

    # Apply extraction to all reads
    metrics = reads_df["CIGAR"].apply(_metrics_from_cigar)

    # Unpack into separate columns
    cigar_metrics = pd.DataFrame(
        metrics.tolist(),
        columns=["CIGAR_N", "total_cigar_N_length"],
        index=reads_df.index,
    )

    cigar_metrics["read_id"] = reads_df["read_id"].values

    return cigar_metrics[["read_id", "CIGAR_N", "total_cigar_N_length"]]
...
def filter_by_cigar(
    ...
    intronic_noN = intronic_reads[
        intronic_reads["read_id"].isin(reads_without_N_set)
    ].copy()
    intronic_reads_with_N = intronic_reads[
        intronic_reads["read_id"].isin(reads_with_N_set)
    ].copy()

    mem_mb = _get_memory_usage()
    logger.info(
        f"[STEP 1] Split reads by CIGAR N: {len(intronic_reads_with_N)} with N, {len(intronic_noN)} without N | Memory: {mem_mb:.1f} MB"
    )
    ...
    mem_mb = _get_memory_usage()
    logger.info(
        f"[STEP 2] Built CIGAR dictionary: {len(cigar_by_read)} reads | Memory: {mem_mb:.1f} MB"
    )
    ...
    annotated_by_combo = {}
    for (read_id, transcript_id), group in intronic_reads_with_N.groupby(
        ...
        annotated_by_combo[(read_id, transcript_id)] = set(
            zip(group["start_intron"], group["end_intron"])
        )
    ...
    read_tx_to_rs = (
        intronic_reads_with_N.groupby(["read_id", "transcript_id"])["rs_id"]
        .first()
        .to_dict()
    )
    ...
    mem_mb = _get_memory_usage()
    logger.info(
        f"[STEP 3] Built annotated dictionary: {len(annotated_by_combo)} combos | Memory: {mem_mb:.1f} MB"
    )
    ...
    rejected_rs_transcript = (
        set()
    )  # Track (rs_id, transcript_id) pairs that fail validation
    ...
    mem_mb = _get_memory_usage()
    logger.info(
        f"[STEP 4] Validated CIGAR introns: {len(rejected_rs_transcript)} (rs_id, tx_id) pairs rejected | Memory: {mem_mb:.1f} MB"
    )
    ...
    mem_mb = _get_memory_usage()
    logger.info(
        f"[STEP 5] Filtered invalid pairs: {len(intronic_reads_with_N)} rows retained | Memory: {mem_mb:.1f} MB"
    )
    ...
    if not intronic_reads_with_N.empty:
        intronic_N = intronic_reads_with_N[
            [
                "read_id",
                "transcript_id",
                "pas_id",
    ...
    mem_mb = _get_memory_usage()
    logger.info(
        f"[STEP 6] Prepared intronic_N: {len(intronic_N)} rows | Memory: {mem_mb:.1f} MB"
    )
    ...
    if not intronic_noN.empty:
        intronic_noN_small = intronic_noN[
            [
                "read_id",
                "transcript_id",
                "pas_id",
        ...
        intronic_noN_small = pd.DataFrame(
            columns=[
                "read_id",
                "transcript_id",
                "pas_id",
    ...
    if filtered_reads is not None and not filtered_reads.empty:
        ...
        intronic_combos_df = pd.concat(
            [intronic_N, intronic_noN_small], ignore_index=True
        )
        intronic_combos_df = intronic_combos_df[
            ["read_id", "rs_id", "transcript_id"]
        ].copy()
        ...
        merged = filtered_reads.merge(
            intronic_combos_df, on=["read_id", "rs_id", "transcript_id"], how="left"
        )
        # Keep rows where _intronic is NaN (not in intronic reads)
        not_intronic_df = (
            merged[merged["_intronic"].isna()].drop(columns=["_intronic"]).copy()
        )
        ...
        if rejected_rs_transcript:
            ...
            close_reads_slice = merged2[merged2["_merge"] == "left_only"].drop(
                columns=["_merge"]
            )
    ...
    mem_mb = _get_memory_usage()
    logger.info(
        f"[STEP 8] Handled close reads: {len(close_reads_df) if not close_reads_df.empty else 0} rows | Memory: {mem_mb:.1f} MB"
    )
    ...
    final_reads = pd.concat(
        [intronic_N, intronic_noN_small, close_reads_df], ignore_index=True
    )
    ...
    mem_mb = _get_memory_usage()
    logger.info(
        f"[STEP 9] Combined reads: {len(final_reads)} total rows | Memory: {mem_mb:.1f} MB"
    )
    ...
    mem_mb = _get_memory_usage()
    logger.info(
        f"[FINAL] filter_by_cigar completed: {len(final_reads)} reads returned | Memory: {mem_mb:.1f} MB"
    )
